...
function normalizeContent(parsedData) {
  ...
  const normalizedPoints = parsedData.points.map(point => {
    const normalizedX = maxRange === 0 ? 0.5 : (point.x - bounds.min.x) / maxRange
    const normalizedY = maxRange === 0 ? 0.5 : (point.y - bounds.min.y) / maxRange
    const normalizedZ = maxRange === 0 ? 0.5 : (point.z - bounds.min.z) / maxRange
